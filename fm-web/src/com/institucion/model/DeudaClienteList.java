...
public class DeudaClienteList extends GridList {
	...
	public DeudaClienteList() {
		super();
		clienteEJB = BeanFactory.<ClienteEJB>getObject("fmEjbCliente");	

		super.addHeader( setHeaders(I18N.getLabel("client.apellido"),"apellido") ).setWidth("15%");
		super.addHeader(setHeaders(I18N.getLabel("client.nombre"),"nombre")).setWidth("13%");
...
//		super.addHeader(setHeaders(I18N.getLabel("client.estado"),"estado")).setWidth("10%");   // Adeuda
		super.addHeader(setHeaders("Responsable","responsable")).setWidth("9%");   // Adeuda
		super.addHeader(setHeaders("F.venta","FVenta")).setWidth("7%");   // Adeuda
		super.addHeader(setHeaders("Cant Dias","dias")).setWidth("6%");   // Adeuda
		super.addHeader(setHeaders(I18N.getLabel("client.deudaPesos"),"deuda")).setWidth("5%");		
...
//				row.appendChild(listCell);						
				...
				String responsableventa=clienteEJB.getResponsableAdeuda(cliente.getId());
				if(responsableventa != null){
					row.appendChild(new Listcell(responsableventa));		
				}else{
					row.appendChild(new Listcell(" "));
				}

				// falta responsable y fecha de venta
				Date fechaventa=clienteEJB.getFechaVentaAdeuda(cliente.getId());
				if(fechaventa != null){
					Calendar ahoraCal= Calendar.getInstance();
					ahoraCal.setTime(fechaventa);
					...
					int dias=(int) ((fechaFinal.getTime()-fechaventa.getTime())/86400000);
					Listcell lc=new Listcell(String.valueOf(dias) +" días");
					lc.setStyle("color:#FF0000 !important; font-weight:bold; ");
					row.appendChild(lc);	
