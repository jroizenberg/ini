...
public class GastosList extends GridList {
	...
	public GastosList() {
		super();
		super.addHeader(new Listheader("Responsable")).setWidth("14%");
		super.addHeader(new Listheader("Sucursal")).setWidth("10%");
		super.addHeader(new Listheader("Año y mes / Temporada")).setWidth("13%");
		super.addHeader(new Listheader(I18N.getLabel("gastos.list.gasto"))).setWidth("15%");
					...
					if(cliente.getTipoGasto().toInt() ==  GastosEnum.SUELDOS.toInt()){
						if((cliente.getPagaSueldosPorMes() != null && cliente.getPagaSueldosPorMes()) 
							...
							if(cliente.getEmpleado() != null)
								row.appendChild(new Listcell(cliente.getEmpleado().getNombre()+"- "+GastosEnum.toString(cliente.getTipoGasto().toInt())+" - Pago Mensual"));
							else	
								row.appendChild(new Listcell(GastosEnum.toString(cliente.getTipoGasto().toInt())+" - Pago Mensual"));	
						}else{
							...
							if(cliente.getEmpleado() != null)
								row.appendChild(new Listcell(cliente.getEmpleado().getNombre()+"- "+ GastosEnum.toString(cliente.getTipoGasto().toInt())+" Fechas de Trabajo Abonadas:"+fechaNacDesde +" al "+ fechaNacHasta));
							else	
								row.appendChild(new Listcell(GastosEnum.toString(cliente.getTipoGasto().toInt())+" Fechas de Trabajo Abonadas:"+fechaNacDesde +" al "+ fechaNacHasta));						
						}
					}else{
						row.appendChild(new Listcell(GastosEnum.toString(cliente.getTipoGasto().toInt())));	
					}
				}else
					row.appendChild(new Listcell(" "));
