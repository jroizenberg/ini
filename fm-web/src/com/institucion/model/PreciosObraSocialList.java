...
public class PreciosObraSocialList extends PanelRoutingCrud<ObraSocialesPrecio> {
	private static final long serialVersionUID = 1L;
	
	public PreciosObraSocialList() {
		super();
		this.setCheckbox();
		this.setUIEvent();
	...
	protected boolean setPanelTitle() {
		...
		this.addAuxheader("Disponible", 1,1);
	...
	public void setField(List<ObraSocialesPrecio> productosList) {
		cleanRows();

		if(productosList != null){
			for (ObraSocialesPrecio prod : productosList) {
				...
				if(prod.getDisponible() != null && prod.getDisponible())
					this.addField(getComboBox(prod.getDisponible()));
				else
					this.addField(getComboBox(false));
			
				if(prod.getSePagaUnaUnicaVez() != null && prod.getSePagaUnaUnicaVez())
					this.addField(getComboBox(prod.getSePagaUnaUnicaVez()));
				else
					this.addField(getComboBox(false));
				
				Intbox precio=new Intbox(prod.getPrecioQuePagaElCliente());
				precio.setAttribute("pagaCliente", true);
				this.addField(precio);  
				Doublebox doub=new Doublebox(prod.getPrecioQuePagaLaObraSocial());
				doub.setFormat("#.##");
				this.addField(doub);  
