...
import com.institucion.model.SueldosFilter;
import com.institucion.model.SueldosList;

public class SueldosSelectorComposer extends SelectorFEComposer implements SucursalDelegate{
	
	private SueldosList clientepanelListGrid;
	...
	public SueldosSelectorComposer(){
		super();
		gastosEJB = BeanFactory.<GastosEJB>getObject("fmEjbGastos");
	}

	private SueldosFilter getClientFilter() {
		return (SueldosFilter)(filter.getGridFilter());
	}
...
//			getClientFilter().getTipoGastoCB().setSelectedIndex(0);
	...
	public void onUpdate(Event event) throws Exception{
		if (hasSelectedOneItem(clientepanelListGrid)){
			Sessions.getCurrent().setAttribute("idSueldo", clientepanelListGrid.getSelectedItem().getValue());		
			super.saveHistory();
			super.gotoPage("/institucion/sueldos-crud.zul");		
		}
	}
	...
	public void onInsert(Event event) throws Exception {
		com.institucion.fm.conf.Session.setAttribute("idSueldo", null);
		super.saveHistory();
		super.gotoPage("/institucion/sueldos-crud.zul");
	}
